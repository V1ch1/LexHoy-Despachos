 ...
 * Usa la configuración de WordPress
 */
...
try {
    ...
    echo "📊 Configuración de Algolia:\n";
    echo "App ID: {$app_id}\n";
    echo "Admin API Key: " . ($admin_api_key ? substr($admin_api_key, 0, 8) . '...' : 'No configurado') . "\n";
    echo "Index Name: {$index_name}\n\n";
    
    if (empty($app_id) || empty($admin_api_key) || empty($index_name)) {
        throw new Exception("Configuración de Algolia incompleta en WordPress");
    }
    
    // Inicializar cliente de Algolia
    $algolia_client = new LexhoyAlgoliaClient($app_id, $admin_api_key, $search_api_key, $index_name);
